...
# 햄버거 아이콘을 클릭할 때 메뉴를 토글하는 함수
def toggle_menu():
    menu_state = st.session_state.menu_state
    st.session_state.menu_state = not menu_state

def main():
    st.set_page_config(page_title="Simple Chatbot with KZN", layout="wide")
    st.title("KZN 챗봇")

    st.sidebar.header("☰ Global Navigation Bar")
    gnb_items = ["KZN", "DOCS", "PRICING", "Contact US"]
    ...
    with st.sidebar:
        openai.api_key = st.text_input(label="OPENAI API Key", placeholder="Enter Your API Key", value="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", type="password")
